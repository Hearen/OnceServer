...
public class Types {
	...
	static Map checkResponse(Map response) throws XenAPIException,
		...
		if (response.get("Status").equals("Failure")) {
			...
			String[] ErrorDescription = ObjectArrayToStringArray((Object[]) response
					.get("ErrorDescription"));
			System.out.println(ErrorDescription[0]);
	...
	public enum VmPowerState {
		...
		private static VmPowerState valueOf(int value)
		{
			switch(value)
			{
			case 0:
...
//					"The server failed to handle your request, due to an internal error.  The given message may give details useful for debugging the problem.");
	...
	public static Types.VmPowerState toVmPowerState(Object object) {
		...
		try {
			return VmPowerState.valueOf(Integer.parseInt(object.toString()));
		} catch (IllegalArgumentException ex) {
	...
	public static Map<String, VBD.Record> toMapOfVBDVBDRecord(Object object) {
		if (object == null) {
			return null;
		}
		Map map = (Map) object;
		Map<String, VBD.Record> result = new HashMap<String, VBD.Record>();
		Set<Map.Entry> entries = map.entrySet();
...
//		record.uuid = toString(map.get("uuid"));//ToDo
		// record.allowedOperations =
		// toSetOfVbdOperations(map.get("allowed_operations"));
		// record.currentOperations =
		// toMapOfStringVbdOperations(map.get("current_operations"));
//		record.VM = toVM(map.get("VM"));//ToDo
//		record.VDI = toVDI(map.get("VDI"));//ToDo
		record.device = toString(map.get("device"));
		record.targetdevice = toString(map.get("targetdevice"));//hda hdb
		record.bootable = intToBoolean(map.get("bootable"));//sites
		record.mode = toVbdMode(map.get("mode"));//r,w,x
		record.type = toVbdType(map.get("type"));//format - aio - raw
...
//		record.ioWriteKbs = toDouble(map.get("io_write_kbs"));
	...
	public static VIF.Record toVIFRecord(Object object) {
		...
		record.device = toString(map.get("device"));//name string
		record.network = toNetwork(map.get("network")); //
//		record.VM = toVM(map.get("VM"));//uuid string
		record.MAC = toString(map.get("MAC"));//string
//		record.MTU = toLong(map.get("MTU"));
//		record.otherConfig = toMapOfStringString(map.get("other_config"));
//		record.currentlyAttached = toBoolean(map.get("currently_attached"));
//		record.statusCode = toLong(map.get("status_code"));
