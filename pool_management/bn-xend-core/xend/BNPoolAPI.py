...
#         log.debug('return from backup %s' % pprint.pformat(response['Value']))
    ...
    def run(self):
        print("I am in the master monitor now really")
        try:
            ...
            while BNPoolAPI._isMaster:
                ...
                if self.verify_backup():
                    log.debug("backup host is still alive")
                    time.sleep(BNPoolAPI._interval)
                    continue



                log.debug("trying to find a backup host")
                ...
                '''
                ToDo added by LHearen 
                E-mail: LHearen@126.com
                Can this selection process be more beautiful?
                '''
                for host in BNPoolAPI.get_hosts():
                    if cmp(host, XendNode.instance().uuid) == 0:
                        continue
                ...
                if BNPoolAPI._backup:
                    log.debug("backup host found, start to copy to backup.")
                    self.copy_to_backup()
                else:
                    log.debug("backup host not available in the current pool, another try will run in 3s.")
...
class BackupMonitorClass(threading.Thread):
    def __init__(self):#, ip, ref):
        ...
        self.log.debug("backup know master ip is: %s" % self.master_ip)
...
#             self.log.debug("rsync with master!")
    ...
    def get_vm_sxp_location(self, vm_ref):
        '''
        find the vm's sxp config file based on vm ref
        '''
        ha_path_map = XendNode.instance().get_ha_sr_location()
        if ha_path_map:
            ha_path = ha_path_map[ha_path_map.keys()[0]]
        ...
        if len(tmp) > 0:
            ...
            self.log.warn("vm_sxp_location not found")
            return ""
...
#                 self.log.debug("checking master state")
                if self.verify_master():
                    self.log.debug("master still online, another check will be in 3s.")
                    time.sleep(3)
                    continue
...
#                     self.log.debug(crushed_vms)
                    ...
                    self.log.debug("HA is disabled, needless to restore crushed VMs")

                # backup become new master
                BNPoolAPI.pool_make_master()
                self.log.debug("backup host becomes the new master now.")
                break
