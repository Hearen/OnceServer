...
##         self.save_PIFs()
    ...
    def get_usb_scsi(self):
        cmd = 'ls -l /dev/disk/by-path | grep usb | awk -F/ \'/sd[b-z][1-9]/{print $3}\''
        ...
        if rc != 0:
            ...
            stdout.close();
            stderr.close();
            raise Exception, 'Failed to get usb infomation: %s.' % err;
        for line in stdout:
...
#        log.debug(cmd)
    ...
    def is_usb_scsi_in_use(self, usb_scsi):
        #cmd = 'grep -r dev %s' % xendoptions().get_xend_domains_path()
        cmd = "grep -r '%s' %s" % (usb_scsi, xendoptions().get_xend_domains_path())
#        log.debug(cmd)
        (rc, stdout, stderr) = doexec(cmd)
        retv = None
        used = False
        if rc != 0:
            ...
            if not err:
                ...
                raise Exception, 'Failed to get usb_scsi in use: %s.' % err;
        retv_s = re.search('(\S+)$',stdout.read())
        if retv_s:
            retv = retv_s.group(1)
        if retv:
    ...
    def check_sr_free_space(self, sr_uuid, vdi_size):
        try:
            ...
            if sr:
                ...
                util = sr.get_physical_utilisation()
                free_space = int(size) - int(util)
