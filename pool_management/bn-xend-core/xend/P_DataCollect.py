...
#         self.host_pifs_metrics = [self.host.get_pif_metrics(pif) for pif in self.host_pifs]
    ...
    def getdata(self):
        ...
        # host 
        log.debug("Get data from memory, current time is: %s" % self.timestamp)
        host_metrics = []       
...
#         f.close()
    ...
    def writetoDB(self, data_type, tables, winsize, all_data):   
        ...
        t = 0
        if count >= winsize:
            for i in xrange(count - winsize + 1):
                log.debug("%s winsize is out of %d:" % (data_type, winsize))
                t = self.session.query(func.min(Cpu.t)).filter(Cpu.id==hostid).scalar()
#                 log.debug(t)
                try:
#                     log.debug(t)
#                     log.debug(self.session.query(Cpu).filter(Cpu.t == t))
...
#                     self.session.commit()
        ...
        try:
            ...
            self.session.commit()
            count = self.session.query(func.count(distinct(Cpu.t))).filter(Cpu.id==hostid).scalar()
            log.debug("After insert %s winsize is %d" % (data_type, count))
            return 
        except Exception:
...
class Domain:
    ...
    def get_memory_free_via_serial(self, vm):
        try:
            ...
            if retv == -1:
                log.error('Get memroy via serial failed: %s' % vm.getName())
                return 0
            return retv * KB
...
#                     log.debug(time.strftime("%Y-%m-%d %A %X %Z", time.localtime()))
            ...
            log.debug("One loop cost time:%0.2f" % (end-s1))
            time.sleep(30 - float(end - s1))
            continue
#           log.debug("after insert")
