...
#                    return self._VDI_clone(session, vdi_uuid_map, True) 
    ...
    #            log.debug(vdi)
                ...
                log.debug('File %s exists: %s' % (tmp[len(tmp)-1], str(exists)))
                log.debug('-------------2----------')
                if not exists:
                    log.debug('-------------3----------')
                    ...
                    log.debug('------------else-----------')
                    if xennode.srs[sr_ref].type in COPY_FROM_SSH_SR:
                        vdi_uuid = xennode.srs[sr_ref].copy_vdi_from_ssh(vdi_struct, vdi_ref, False, False)
                    else:
                        vdi_uuid = xennode.srs[sr_ref].copy_vdi(vdi_struct, vdi_ref, False, False)
